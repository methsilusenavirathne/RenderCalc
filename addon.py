...
BASE_GPU_PRICE_JSON_URL = "https://raw.githubusercontent.com/methsilusenavirathne/_GPU/main/gpu.json"

def get_base_gpu_price_from_server(Base_Gpu_price):
    try:
        with urllib.request.urlopen(BASE_GPU_PRICE_JSON_URL) as response:
            data = response.read().decode('utf-8')
            if response.status == 200:
                return float(json.loads(data).get("base_gpu_price", Base_Gpu_price))
    except urllib.error.URLError as e:
        print("Error fetching base GPU price from the server:", str(e))
    return Base_Gpu_price
...
def cost_calculation():
    ...
    Base_Gpu_price = get_base_gpu_price_from_server(Base_Gpu_power)  # Fixed this line
...
class Addon_Panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = layout.row()
        row.operator("wm.url_open", text="Render for Low cost", icon='WORLD').url = "https://www.fiverr.com/boofmerexterien/render-your-blender-projects-instantly-for-lowest-rates"
